...
function updateFinancialCards(data) {
    ...
    // Removido, pois não há elemento com este ID no HTML fornecido
...
function initializeDashboardTabs() {
    ...
    const switchTab = async (targetTab) => { // Tornar a função assíncrona
        ...
        // Ou se for a aba de Análise Financeira, garante que os dados sejam carregados.
        const isLoaded = targetTab.dataset.loaded === 'true';
        if (!isLoaded || targetContentId === 'content-analise-financeira') { // Adicionada condição para recarregar ou carregar Análise Financeira
            if (targetContentId === 'content-visao-geral') {
                loadVisaoGeralData();
            } else if (targetContentId === 'content-analise-financeira') {
                // Obter as datas dos inputs já definidos pelo renderGerencialDashboard
                const startDateInput = document.getElementById('gerencial-start-date');
                const endDateInput = document.getElementById('gerencial-end-date');
                ...
                if (startDateInput && endDateInput && startDateInput.value && endDateInput.value) {
                    ...
                    // Se por algum motivo as datas não estiverem nos inputs, define para o mês atual
                    const today = new Date();
                    startDate = new Date(today.getFullYear(), today.getMonth(), 1);
                    endDate = new Date(today.getFullYear(), today.getMonth() + 1, 0);
                }
                
                await loadAndRenderDashboardData(startDate, endDate); // Chama o carregamento de dados
