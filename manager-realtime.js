...
async function loadVisaoGeralData() {
    ...
    try {
        ...
        const ordersRef = collection(db, "orders");
        const financialQuery = query(ordersRef, where("createdAt", ">=", Timestamp.fromDate(sevenDaysAgo)));
...
function initializeDashboardTabs() {
    ...
    const switchTab = (targetTab) => {
        ...
        if (!isLoaded) {
            if (targetContentId === 'content-visao-geral') loadVisaoGeralData();
            else if (targetContentId === 'content-analise-produtos') renderProductAnalysisDashboard();
            else if (targetContentId === 'content-analise-clientes') populateCustomerAnalysisData();
