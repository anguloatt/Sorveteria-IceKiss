    // public/utils.js (Corrigido)

    // utils.js - Funções utilitárias e de interface (toasts, modais, formatação)

    import { dom } from './domRefs.js'; // Importa o objeto dom centralizado
    import { managerCredentials, masterCredentials, productsConfig, storeSettings, currentReminderOrders } from './app.js'; // Importa credenciais, configurações e a lista de lembretes do app.js

    // CORREÇÃO CRÍTICA: Removendo importações diretas para date-fns e locale
    // A versão CDN (cdn.min.js) expõe as funções e o locale via o objeto global 'dateFns'.
    // Não é necessário importar como módulo, pois o script já o torna global.

    // Variável global para controlar o timeout do toast
    let toastTimeout;
    ...
    export function showToast(message, type = 'info', duration = 3000) {
        ...
        // Limpa qualquer timeout existente para que o novo toast apareça imediatamente
        clearTimeout(toastTimeout);

        dom.toast.textContent = message; // Define a mensagem do toast
        dom.toast.className = `toast ${type}`; // Define as classes CSS para estilo e tipo

        // Força o reflow para garantir que a transição CSS seja aplicada
        void dom.toast.offsetWidth;

        dom.toast.classList.add('show'); // Adiciona a classe 'show' para exibir o toast

        // Define um timeout para esconder o toast após a duração especificada
        toastTimeout = setTimeout(() => {
            dom.toast.classList.remove('show'); // Remove a classe 'show' para esconder o toast
        }, duration);
    }
     ...
     * - {boolean} showInput Se deve exibir campos de input de usuário/senha.
     * - {boolean} passwordRequired Se a senha é obrigatória e deve ser validada contra as credenciais do gerente/mestra.
     * @returns {Promise<boolean|object>} Retorna `false` se cancelado. Se confirmado, retorna `true` (para confirmações simples) ou um objeto `{ confirmed: true, user: '...', pass: '...' }` se `showInput` for usado.
     */
    export function showCustomConfirm(title, message, options = {}) {
        return new Promise((resolve) => {
            // Define valores padrão para as opções para garantir que sempre existam
            ...
            // Limpa os campos de input e esconde-os por padrão
            confirmModalInputUser.value = '';
            confirmModalInputPass.value = '';

            // Mostra e configura os inputs se a opção for ativada
            if (finalOptions.showInput) {
                confirmModalInputContainer.classList.remove('hidden');
                // Lógica para diferentes tipos de input
                if (finalOptions.showInput === 'pass-only') {
                    confirmModalInputUser.classList.add('hidden'); // Esconde o campo de usuário
                    confirmModalInputPass.classList.remove('hidden');
                    confirmModalInputPass.placeholder = "Digite a senha"; // Placeholder genérico
                    confirmModalInputPass.focus();
                } else { // Padrão 'user-pass'
                    confirmModalInputUser.classList.remove('hidden');
                    confirmModalInputPass.classList.remove('hidden');
                    confirmModalInputUser.placeholder = "Usuário"; // Placeholder padrão
                    confirmModalInputPass.placeholder = "Senha"; // Placeholder padrão
            ...
            const handleConfirm = async () => {
                if (finalOptions.passwordRequired) {
                    ...
                    if ((user.toLowerCase() === managerCredentials.user && pass === managerCredentials.pass) ||
                        ...
                        // Não resolve a promise aqui, mantém o modal aberto para nova tentativa
                    }
                } else if (finalOptions.showInput) {
                    cleanupAndResolve({
                        confirmed: true, // CORREÇÃO: Adiciona a flag de confirmação que estava faltando
            ...
            // Função para limpar todos os listeners e resolver a promise
            ...
            // Adiciona listener para a tecla Enter nos campos de input do modal
    ...
    /**
     * Converte uma string de moeda brasileira (R$ X.XXX,XX) para um número (float).
     * @param {string} currencyString A string de moeda.
    ...
    export function parseCurrency(currencyString) {
        ...
        const cleanedString = currencyString.replace(/[R$]/g, '').replace(/\./g, '').replace(',', '.').trim();
    ...
    export function updateWeekdayDisplay(dateString) {
        ...
        // O new Date('YYYY-MM-DD') cria a data em UTC. Ao usar getDay(), ele pode retornar o dia anterior.
        // A forma abaixo cria a data corretamente no fuso horário local, garantindo que o dia da semana seja o correto.
    ...
    export function formatTime(e) {
        ...
        // 1. Remove tudo que não é dígito para limpar espaços e caracteres inválidos.
        let value = input.value.replace(/\D/g, '');

        // 2. Valida a hora (primeiros 2 dígitos) para não ser maior que 23.
        if (value.length >= 2) {
            let hour = parseInt(value.substring(0, 2), 10);
            if (hour > 23) {
                value = '23' + value.substring(2); // Corrige para 23h e mantém o resto
            }
        }

        // 3. Valida os minutos (terceiro e quarto dígitos) para não ser maior que 59.
        if (value.length >= 4) {
            let minute = parseInt(value.substring(2, 4), 10);
            if (minute > 59) {
                value = value.substring(0, 2) + '59'; // Corrige para 59min
            }
        }

        // 4. Adiciona a máscara ":" automaticamente e limita o tamanho.
        if (value.length > 2) {
     ...
     * Esta função é exclusiva para a saída de texto simples.
    ...
    export function generateTicketText(order) {
        ...
        const width = 40; // Largura do cupom em caracteres
        ...
        // Cabeçalho dos itens
        if (printUnitPrice) {
            text += `${leftAlignText("ITEM (QTD X VL.UNIT)", width)}\n`;
        } else {
            text += `${twoColumns("ITEM", "TOTAL", width)}\n`;
        }
        text += `${thinSeparator}\n`;

        let totalSalgados = 0;

        // Lista os itens sem agrupar por categoria
        (order.items || []).forEach(item => {
            ...
            // Trata nomes de produtos longos para não quebrar o layout
            ...
            // Inclui 'revenda' no total de salgados do ticket, conforme a lógica de negócio.
        ...
        text += `${centerText(`STATUS: ${paymentStatusText}`, width)}\n`;
        text += `${separator}\n`;
        text += `${centerText(footerMessage, width)}\n`;
        text += `\n\n`; // Linhas extras para espaçamento final
    ...
    export function generatePrintableReminderText(orders) {
        ...
        orders.forEach(order => {
            (order.items || []).forEach(item => {
                ...
                // Inclui 'revenda' no total de salgados do lembrete, conforme a lógica de negócio.
        ...
        if (Object.keys(consolidatedItems).length > 0) {
            ...
            Object.entries(consolidatedItems).sort((a, b) => b[1] - a[1]).forEach(([name, count]) => {
                text += `${String(count).padEnd(5)} ${name}\n`; // Ajustado para 5 espaços para quantidade
            });
        } else {
    ...
    export function printTicket(order) {
        ...
        // Usa a função generateTicketText para obter o texto formatado para impressão
            ...
            <head>
                ...
                <style>
                    body {
                        ...
                        font-size: 12px; /* Tamanho de fonte típico de cupom */
                        margin: 0; /* Remove margens padrão */
                        padding: 5px; /* Pequeno padding para o conteúdo */
                        width: 80mm; /* Largura típica de cupom (ajuste se sua impressora for diferente) */
                        box-sizing: border-box;
                    }
                    pre {
                        white-space: pre-wrap; /* Quebra de linha para textos longos */
    ...
    export function sendWhatsAppMessage(order) {
        ...
        // Usa a função generateTicketText para obter o texto formatado para WhatsApp
    ...
    export function printElement(elementId, title = document.title) {
        ...
        // Clona o conteúdo para manipulação sem afetar o DOM original
        const clonedContent = printContent.cloneNode(true);

        // Remove elementos com a classe 'no-print' do conteúdo clonado
        clonedContent.querySelectorAll('.no-print').forEach(el => el.remove());

        // Prepara a janela de impressão
        const printWindow = window.open('', '_blank');
        if (!printWindow) {
            showToast("Permita pop-ups para imprimir.", "error");
            return;
        ...
        printWindow.document.write(`
            ...
            <head>
                ...
                <style>
                    ...
                    /* Estilos para o modal de lembrete de produção */
                    #reminder-content-to-print {
                        padding: 10px;
                    ...
                    #reminder-summary-items div, #reminder-orders-list div {
                        border: none; /* Remove bordas para impressão */
                        padding: 0;
                        margin-bottom: 0;
                    }
                    h2, h3 { color: #333; }
    ...
    export function printReminderList() {
        // Usa a nova função para gerar o texto formatado para impressão
    ...
    export function formatInputAsCurrency(inputOrValue) {
        try {
            ...
            // Pega o valor, seja do elemento ou da string direta
            if (isInputElement) {
            ...
            // Se for um elemento de input, atualiza o valor dele
            if (isInputElement) {
                inputOrValue.value = formattedValue;
            }

            return formattedValue; // Sempre retorna o valor formatado
        } catch (error) {
    ...
    /**
     * NOVO: Formata a data da primeira compra para uma string amigável ("Cliente há X meses").
    ...
    export function formatClientSince(firstOrderDate) {
        ...
        // CORREÇÃO: Acessa as funções e o locale via o objeto global 'dateFns'
        // Verifica se window.dateFns e window.dateFns.locale.ptBR existem antes de usar
        if (typeof window.dateFns === 'undefined' || typeof window.dateFns.locale === 'undefined' || typeof window.dateFns.locale.ptBR === 'undefined') {
            ...
            return formatDateToBR(firstOrderDate); // Fallback para formato de data simples
        }
    ...
    export function getSalgadosCountFromItems(items) {
        ...
        return items.reduce((total, item) => {
            // CORREÇÃO: Inclui a categoria 'revenda' na contagem de salgados para a agenda de horários,
            // conforme solicitado, para que pedidos como o 1556 apareçam corretamente.
    ...
    export function playSound(soundUrl) {
        ...
        audio.play().catch(error => {
            // O erro "The request is not allowed by the user agent" é comum
            // porque os navegadores bloqueiam a reprodução automática de som
            // antes de uma interação do usuário. Isso é esperado e não quebra a aplicação.
